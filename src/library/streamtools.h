...
void StreamDataWidthConverter(
		...
		int inWidth, int outWidth, int input_num) {
//#pragma HLS inline
	if (in_data_width > out_data_width) {
        PackedStencil<T, in_data_width, 1, 1, 1> inData;
        size_t len = out_data_width * sizeof(T) << 3;
...
#pragma HLS PIPELINE II=1
                ...
                PackedStencil<T, out_data_width, 1, 1, 1> outData;
			    //for (int ii = 0; ii < outWidth; ii++)
                //    outData(ii, 0, 0, 0) = inData(ii, 0, 0, 0);
                outData.value = inData.value(len-1, 0);
                inData.value >>= len;
                out.write(outData);
		    }
        }
