...
#pragma HLS INTERFACE m_axi port = data_out offset = slave bundle = gmem depth = 961
...
#pragma HLS dataflow
    ...
    uint16_t rng_in_2d[3] = {2, IMG_SIZE, IMG_SIZE>>1};
    uint16_t st_in_2d[3] = {0, 1, 0};
    uint16_t rng_out_2d[2] = {IMG_SIZE, (IMG_SIZE-2)>>1};
    ...
    read_start(0) = 0;
    read_start(1) = 1;
    uint16_t rng_write_bank_2d[3] = {2, IMG_SIZE, IMG_SIZE>>1};
    uint16_t st_write_bank_2d[3] = {1, 0, 0};
    uint16_t rng_read_bank_2d[2] = {IMG_SIZE, IMG_SIZE>>1};
...
#pragma HLS RESOURCE variable=st_read_bank_2d core=RAM_1P_LUTRAM
    ...
    AddrGenTemp<3>(addr_in_2D, IMG_SIZE*IMG_SIZE, rng_in_2d, st_in_2d);
    AddrGenTemp<2>(addr_out_2D, IMG_SIZE*(IMG_SIZE-2)>>1, rng_out_2d, st_out_2d);

    BankIDGenCircular<uint32_t, 3, 1, 1, 1, 1>(bank_in_2D, write_start, IMG_SIZE*IMG_SIZE, 2, rng_write_bank_2d, st_write_bank_2d);
    BankIDGenCircular<uint32_t, 2, 2, 1, 1, 1>(bank_out_2D, read_start, IMG_SIZE*(IMG_SIZE-2)>>1, 2, rng_read_bank_2d, st_read_bank_2d);

    NDShiftReg<IMG_SIZE, 2, DATAWIDTH, 1, 1, 1, DATAWIDTH, 1, 3, 1, dtype>::call(inStream, intermStream, bank_in_2D, bank_out_2D, addr_in_2D, addr_out_2D, 2*IMG_SIZE, IMG_SIZE*IMG_SIZE, 2);
    for (int i = 0; i < (IMG_SIZE-2)>>1; i ++)
        NDShiftReg<1, 3, DATAWIDTH, 1, 3, 1, DATAWIDTH, 3, 3, 1, dtype>::call(intermStream, outStream, 2, IMG_SIZE, 2);

    write_result(data_out, outStream, ((IMG_SIZE-2)*(IMG_SIZE-2))>>2);
}
