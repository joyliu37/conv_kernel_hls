...
int main() {
    ...
    static PackedStencil<dtype, DATAWIDTH, 1, 1, 1> HLSout[4*READ_SIZE/DATAWIDTH];
    ...
    //HLS kernel
    top(image, HLSout, RAM_SIZE/DATAWIDTH, READ_SIZE/DATAWIDTH);
    ...
    //check if matching
    int err_cnt = 0;
    for (int i = 0; i < 4*READ_SIZE/DATAWIDTH; i ++) {
        for (int ii = 0; ii < DATAWIDTH; ii ++) {
