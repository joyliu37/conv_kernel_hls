...
template<typename T>
void Mem2Stream(T* _feature, hls::stream<T> &out, layerPara para, tilingID iter){
#pragma HLS inline

//TODO: put off_beg and off_end into a file
load_feature2Stream: for (int input_y = 0 - iter.tilingIDy; input_y < Y_SZ + 1- iter.tilingIDy; input_y ++){
#pragma HLS LOOP_TRIPCOUNT max=18
                  for(int input_x = 0 - iter.tilingIDx; input_x < X_SZ + 1 - iter.tilingIDx; input_x ++ ){
#pragma HLS LOOP_TRIPCOUNT max=18
...
#pragma HLS PIPELINE II=1
                              ...
                              int32_t ddrAddr = ddrC +\
									  ...
									  (input_y + iter.tilingIDy*Y_SZ) * para.Chin * para.Width;

                              T f = ((T *) _feature)[ddrAddr];
                              out.write(f);
                          }
                      }
                  }
...
template<typename T>
void StreamPad(hls::stream<T> &in, hls::stream<T> &out, layerPara para, tilingID iter){
#pragma HLS inline
    ...
    T out_data, in_data;
stream_pad: for (int input_y = 0; input_y < Y_SZ + para.Ksz- 1; input_y ++){
...
#pragma HLS inline off
        ...
        void loadFromDRAM(hls::stream<T> &_feature_stream, T* _feature_buf, layerPara para, tilingID iter);//TODO come up with all the parameter needed by load
        ...
        void call(hls::stream<T> &in, hls::stream< PackedStencil<T, P_CIN, 1, 1, 1> > & out_stream,
                layerPara para, tilingID iter);

        void call_start(hls::stream<T> &in, layerPara para, tilingID iter);
};
...
template <typename T>
void Doublebuffer_feature<T>::call(hls::stream<T> &in, hls::stream< PackedStencil<T, P_CIN, 1, 1, 1> > & out_stream,
                layerPara para, tilingID iter){
#pragma HLS inline
            if(flag){
...
template <typename T>
void Doublebuffer_feature<T>::call_start(hls::stream<T> &in, layerPara para, tilingID iter){
#pragma HLS inline
                this->loadFromDRAM(in, _db_0, para, iter);
                cnt += 1;
}
...
template <typename T>
void Doublebuffer_feature<T>::loadFromDRAM(hls::stream<T> &_feature_stream, T* _feature_buf, layerPara para, tilingID iter){
#pragma HLS inline off
