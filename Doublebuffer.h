...
#pragma HLS inline off
	...
	void loadFromDRAM(
			...
			T* _feature_buf, layerPara para, tilingID iter); //TODO come up with all the parameter needed by load
	void feedStream(T* _feature_buf, layerPara para,
...
void Doublebuffer_feature<T, data_width>::loadFromDRAM(
		hls::stream<PackedStencil<T, data_width, 1, 1, 1>> &_feature_stream,
		T* _feature_buf, layerPara para, tilingID iter) {
#pragma HLS inline off
...
#pragma HLS PIPELINE II=1
 ...
 input_y * Cin_SZ * (X_SZ + para.Ksz - 1);
				...
				_feature_buf[buffAddr] = data(0, 0, 0, 0);
			}
			//normal situation to move feature map
		}
	}
...
template<typename T, int data_width>
void Doublebuffer_feature<T, data_width>::feedStream(T* _feature_buf,
		layerPara para,
		hls::stream<PackedStencil<T, P_CIN, 1, 1, 1> > & out_stream) {
#pragma HLS inline off
	//if(this-> empty[flag])
